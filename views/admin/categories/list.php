...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem">
    <div>
        <h1 style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.8rem; font-weight:600; color:#1a1a1a; margin-bottom:0.25rem">Categories</h1>
        <p style="color:#666; font-size:0.9rem"><?= count($categories) ?> categor<?= count($categories) !== 1 ? 'ies' : 'y' ?></p>
    </div>
</div>
            ...
            <tbody>
                ...
                <tr x-data="<?= htmlspecialchars(json_encode([
                    ...
                    <td style="text-align:center">
                        ...
                        <span class="badge badge-sent"><?= (int) $cat['product_count'] ?></span>
                        <?php else: ?>
                    ...
                    <td style="padding-right:1.5rem; white-space:nowrap">
                        ...
                        <form method="POST"
                              action="/admin/categories/<?= (int) $cat['id'] ?>"
                              x-show="editing"
                        ...
                        <?php if ((int) $cat['product_count'] === 0): ?>
                        <form method="POST"
                              action="/admin/categories/<?= (int) $cat['id'] ?>/delete"
                              style="display:inline-block; margin-left:0.375rem"
                              x-show="!editing"
                              data-confirm="Delete category &quot;<?= htmlspecialchars(addslashes($cat['name_en'])) ?>&quot;? This cannot be undone.">
                            <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($csrfToken ?? '') ?>">
                            <button type="submit"
                                    class="admin-btn"
                                    style="padding:0.4rem 0.875rem; font-size:0.7rem;
                        ...
                        <?php else: ?>
                        <span x-show="!editing"
                              style="font-size:0.75rem; color:#aaa; margin-left:0.5rem"
                              title="Move or delete the <?= (int) $cat['product_count'] ?> product(s) first">
                            (has products)
                        </span>
                        <?php endif; ?>
                ...
                <tr>
                    <td colspan="7" style="text-align:center; padding:3rem 1.5rem; color:var(--color-muted)">
                        No categories yet. Use the form below to add one.
                    </td>
                </tr>
                <?php endif; ?>
            </tbody>
...
<div class="admin-card">
    <p class="admin-card-title" style="margin-bottom:1.25rem">Add Category</p>
    ...
    <form method="POST" action="/admin/categories">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr auto auto auto; gap:0.875rem; align-items:end">
            ...
            <div class="admin-form-group" style="margin-bottom:0">
                ...
                <input type="text" id="new_name_en" name="name_en" required placeholder="Bouquets">
            </div>

            <div class="admin-form-group" style="margin-bottom:0">
                <label for="new_name_es">Name (ES)</label>
                <input type="text" id="new_name_es" name="name_es" placeholder="Ramos">
            </div>

            <div class="admin-form-group" style="margin-bottom:0">
                <label for="new_slug">Slug <span style="color:#e53e3e">*</span></label>
                <input type="text" id="new_slug" name="slug" required placeholder="bouquets"
