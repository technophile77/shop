...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem">
    <div>
        <h1 style="font-family:'Cormorant Garamond',Georgia,serif; font-size:1.8rem; font-weight:600; color:#1a1a1a; margin-bottom:0.25rem">Paper Colors</h1>
        <p style="color:#666; font-size:0.9rem"><?= count($paperColors) ?> color<?= count($paperColors) !== 1 ? 's' : '' ?></p>
    </div>
</div>
            ...
            <tbody>
                ...
                <tr x-data="<?= htmlspecialchars(json_encode([
                    ...
                    <td>
                        ...
                        <input x-show="editing" type="text" x-model="hex" placeholder="#C8A06A"
                               pattern="#[0-9A-Fa-f]{6}"
                               title="Six-digit hex code, e.g. #C8A06A"
                    ...
                    <td style="padding-right:1.5rem; white-space:nowrap">
                        ...
                        <form method="POST"
                              action="/admin/paper-colors/<?= (int) $pc['id'] ?>"
                              x-show="editing"
                        ...
                        <!-- DELETE -->
                        <form method="POST"
                              action="/admin/paper-colors/<?= (int) $pc['id'] ?>/delete"
                              style="display:inline-block; margin-left:0.375rem"
                              x-show="!editing"
                              data-confirm="Delete paper color &quot;<?= htmlspecialchars(addslashes($pc['name_en'])) ?>&quot;? This cannot be undone.">
                            <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($csrfToken ?? '') ?>">
                            <button type="submit"
                                    class="admin-btn"
                                    style="padding:0.4rem 0.875rem; font-size:0.7rem;
                ...
                <tr>
                    <td colspan="6" style="text-align:center; padding:3rem 1.5rem; color:var(--color-muted)">
                        No paper colors yet. Use the form below to add one.
                    </td>
                </tr>
                <?php endif; ?>
            </tbody>
...
<!-- ============================================================
     ADD PAPER COLOR FORM
     ============================================================ -->
<div class="admin-card">
    <p class="admin-card-title" style="margin-bottom:1.25rem">Add Paper Color</p>

    <form method="POST" action="/admin/paper-colors">
        <input type="hidden" name="_csrf_token" value="<?= htmlspecialchars($csrfToken ?? '') ?>">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr auto auto auto auto; gap:0.875rem; align-items:end">
            ...
            <div class="admin-form-group" style="margin-bottom:0">
                ...
                <input type="text" id="new_name_en" name="name_en" required placeholder="Kraft">
            </div>

            <div class="admin-form-group" style="margin-bottom:0">
                <label for="new_name_es">Name (ES)</label>
                <input type="text" id="new_name_es" name="name_es" placeholder="Kraft">
            </div>

            <div class="admin-form-group" style="margin-bottom:0">
                <label for="new_hex">Hex <span style="color:#999; font-weight:400; font-size:0.75rem">(optional)</span></label>
                <input type="text" id="new_hex" name="hex" placeholder="#C8A06A"
                       pattern="#[0-9A-Fa-f]{6}"
                       title="Six-digit hex code, e.g. #C8A06A"
